...
class TestAuditPage(base.Test):
  ...
  def test_assessments_widget_has_correct_page_count(self, selenium,
    ...
    assessments_count = 101
    items_per_page = 10
    expected_page_count = math.ceil(
      float(assessments_count) / float(items_per_page))
    ...
    assert int(expected_page_count) == actual_page_count

  @pytest.mark.parametrize("expected_items_count", [10, 25, 50])
  @pytest.mark.smoke_tests
  def test_assessments_widget_shows_correct_items_count_per_page(self, selenium,
                                                                 rest_create_audit,
                                                                 rest_create_control,
    ...
    selenium.get(environment.APP_URL + url.AUDIT.format(
      rest_create_audit["id"]) + url.Widget.ASSESSMENTS)
    assessments_widget = generic_widget.Assessments(selenium)
    assessments_widget.pagination_control. \
      select_items_per_page(expected_items_count)

    assessments_widget_refreshed = generic_widget.Assessments(selenium)

    assert len(assessments_widget_refreshed.members_listed) == \
           expected_items_count
  ...
  @pytest.mark.smoke_tests
  def test_switch_page_forward_buttons_work_correct_on_assessments_widget(self,
                                                                          selenium,
                                                                          rest_create_audit,
                                                                          rest_create_control,
                                                                          assessments_count,
    ...
    selenium.get(environment.APP_URL + url.AUDIT.format(
      rest_create_audit["id"]) + url.Widget.ASSESSMENTS)
    assessments_widget = generic_widget.Assessments(selenium)
    assessments_widget.pagination_control.select_items_per_page(10)
  ...
  @pytest.mark.smoke_tests
  def test_switch_page_back_buttons_work_correct_on_assessments_widget(self,
                                                                       selenium,
                                                                       rest_create_audit,
                                                                       rest_create_control,
                                                                       assessments_count,
